...
from robot_localization_system import RobotEstimator
...
def main():
    ...
    # Plotting 
    #add visualization of final x, y, trajectory and thet

    base_pos_all = np.array(base_pos_all)  # Assuming this is not already a numpy array
    base_bearing_all = np.array(base_bearing_all)

   # Create a figure with subplots for combined XY and Theta plots side by side

    # Plot X and Y positions over time in the left subplot
    plt.figure()
    plt.subplot(1, 2, 1)
    ...
    # Plot Theta (bearing) over time in the right subplot
    plt.subplot(1, 2, 2)
    plt.plot(base_bearing_all, label="Theta (Bearing)")
    plt.title("Theta (Bearing) Over Time")
    plt.xlabel("Time Step")
