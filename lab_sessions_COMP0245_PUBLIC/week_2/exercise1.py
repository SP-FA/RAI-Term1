...
#     }
# }
# n = 21  # 5  absolute_error: 14, squared_error: 11, friedman_mse: 12~13
# criterions = ["squared_error", "friedman_mse", "absolute_error"]  # R2: abs_err > other, MSE: abs_err < other (a little)
...
#             res_dict[s][c].append([R2, MSE])
#
# plt.figure()
# for c in criterions:
#     for s in splitters:
#         res = np.array(res_dict[s][c])
#         R2 = res[:, 0]
#         plt.plot(R2, label=f"criterion: {c}, splitter: {s}", alpha=0.5)
...
#         res = np.array(res_dict[s][c])
#         MSE = res[:, 1]
#         plt.plot(MSE, label=f"criterion: {c}, splitter: {s}", alpha=0.5)
# plt.xlabel("Max Depth")
# plt.ylabel("MSE")
# plt.legend()
# plt.xticks(np.arange(0, 20, 1))
